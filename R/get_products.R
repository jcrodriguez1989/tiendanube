...
#' @export
#'
get_products <- function(store_id, page, per_page = 200,
...
#' @export
#'
get_all_products <- function(store_id, access_token = Sys.getenv("TIENDANUBE_ACCESS_TOKEN")) {
  page <- 1
  products <- data.frame()
  while (TRUE) {
    new_products <- get_products(store_id, page, access_token = access_token)
    if (isTRUE(new_products$code == 404)) {
      break
    }
